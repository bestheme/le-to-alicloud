...
- 生成时间：2026-09-05T18:34:15Z
- region：`cn-hangzhou`；备用 CAS region：`ap-southeast-1`
- 本文件由 `make test-integration` 生成，不要手改。

| # | 待核实 | 结论 | 证据 |
|---|---|---|---|
| #1 | CAS 对 PKCS#1 / PKCS#8 / SEC1 私钥的接受情况（PKCS#1 RSA） | 接受 | 上传成功，certId=27080715 |
| #1 | CAS 对 PKCS#1 / PKCS#8 / SEC1 私钥的接受情况（SEC1 EC） | 接受 | 上传成功，certId=27080716 |
| #1 | CAS 对 PKCS#1 / PKCS#8 / SEC1 私钥的接受情况（PKCS#8） | 接受 | 上传成功，certId=27080717 |
...
| #3 | CAS ClientToken 语义：同 token 重复上传返回同 certId 还是报错 | 同 token 重复上传报错 | code=NameRepeat class=Permanent first=27080724 |
| #4 | CAS 证书 Name 是否接受 - 与 .（连字符） | 接受 | 上传成功，certId=27080726 云上存的名字=itest_785e60f6fcc1-hyphen |
| #4 | CAS 证书 Name 是否接受 - 与 .（点号） | 接受 | 上传成功，certId=27080727 云上存的名字=itest_e6a61e1f5a12.dot |
| #5 | CAS 是否接受 leaf+intermediate（无 root）以及链顺序是否敏感（leaf + 其签发 CA，两块） | 接受 | 上传成功，certId=27080718 |
| #5 | CAS 是否接受 leaf+intermediate（无 root）以及链顺序是否敏感（intermediate 在前） | 拒绝 | code=NotMatch.CertificateAndPrivateKey class=Permanent |
| #5 | CAS 是否接受 leaf+intermediate（无 root）以及链顺序是否敏感（仅 leaf） | 接受 | 上传成功，certId=27080719 |
...
| #9 | 同账号在不同 CAS endpoint 上传的证书是否互相可见 | 不可见：两个 endpoint 的证书集合互相隔离（双向验证） | 同一份 PEM，上传于 cn-hangzhou 得 certId=27080721、上传于 ap-southeast-1 另得 certId=584597（两个 ID 不在同一量级，是两套独立 ID 空间而非复制延迟）；前者在 ap-southeast-1 列举不可见，后者在本地可见、在 cn-hangzhou 同样不可见 |
