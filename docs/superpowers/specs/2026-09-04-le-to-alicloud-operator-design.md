...
| 15 | OSS `PutCname` 带 `CertId` + `Force=true` 的首绑与换绑是否都成功（T-OSS1）（**未画删除线：只关闭了首绑一半**）。**首绑已核实**（2026-09-07 12:16Z，`www.bestheme.ac.cn` / bucket `applanding-102181`，operator v0.2.0 的生产首绑）：`PutCname{CertId: "27114423-cn-hangzhou", Force: true}` **成功**，Binding 随即 `Applied=True`、`status.appliedCertRef=27114423-cn-hangzhou`。同一次实测还暴露了一个 RAM 前提：这次调用**先返回过 `AccessDenied`**，直到给子账号补上 `yundun-cert:DescribeSSLCertificatePrivateKey` / `DescribeSSLCertificatePublicKeyDetail` / `CreateSSLCertificate` 三个动作才通过（§8.3 已据此更新；同一份策略下 `oss:ListCname` 一直是通的）。**换绑那一半未测**——要等 `www.bestheme.ac.cn` 的下一次续期换代才能观测。本行结论**不来自 `RESULTS.md`**：它是生产首绑而非探针跑出来的，`RESULTS.md` 未为此重新生成 | 换绑被拒 ⇒ 续期永远 ApplyFailed。首绑这一半已确定可行，剩余风险收窄到「带 `PreviousCertId` 语义之外的换代被拒」这一种 |
| 16 | OSS `CertId` 区域后缀取 CAS 区域还是 bucket 区域（T-OSS2；两者同为 cn-hangzhou 时只能证明「同区域可行」）（**未画删除线：跨区那一半没关**）。**只证明了「同区域可行」**：2026-09-07 那次首绑里 CAS 区域与 bucket 区域**同为 `cn-hangzhou`**，`27114423-cn-hangzhou` 这个后缀在两种解释下取值相同，**分辨不出后缀语义**。跨区组合（CAS 与 bucket 不同 region）**仍未核实**，且不打算为它构造实测——需要另一个区域的 bucket，属于所有者已否决的「另建牺牲 bucket」 | 跨区组合下 Apply 被拒。同区域部署不受影响；文档层面按「后缀必须与上传时的 CAS 区域一致」使用，跨区部署前须先实测 |
| 17 | ~~`ListCname` 回报的 `CertId` 是否与写入字符串逐字相同（T-OSS3）~~ **已核实**：**逐字相同**——2026-09-07 那次首绑后用 ossutil 对 bucket `applanding-102181` 读回，`ListCname` 给出的 `CertId` 为 `27114423-cn-hangzhou`，与写入值逐字节一致，因此短路比对一定命中。同一次读回还确认 `ListCname.Owner` **非空**（值为 `102181`，Binding 的 `status.boundAccountId` 记的是同一个），账号 fencing 的前提成立。本行结论同样**不来自 `RESULTS.md`**（生产首绑，非探针） | 短路永不命中 ⇒ 每小时一次无谓 PutCname 并误报漂移。已确认命中，该失效方式排除；账号 fencing 也不落空 |
...
#15–#20 的探针在 `test/integration/oss_test.go`，以 `OSS_TEST_BUCKET` / `OSS_TEST_DOMAIN` 门控；所有者决定（2026-09-07 D24）不另建牺牲 bucket，首次实测由 `www.bestheme.ac.cn` 的受控首绑完成。**那次首绑已于 2026-09-07 12:16Z 做完**（bucket `applanding-102181`，operator v0.2.0），关闭了 **#17**、关掉 **#15 的首绑一半**，并把 **#16** 钉死在「同区域可行」。**这三行的结论不以 `RESULTS.md` 为依据**——它们出自一次生产绑定而不是探针，`RESULTS.md` 未为此重新生成，所以按本节开头的规矩把一手依据写在行内（Binding 的 `status` 与 ossutil 的读回）。相应地，`RESULTS.md` 里 #15 / #16 / #17 的「未实测」记录**已经过时**，下一轮探针跑完自然覆盖，在那之前以本表为准。#18 / #19 / #20 未被这次首绑触及：首绑不走 Unbind、不删被引用的证书，而那次 `AccessDenied` 缺的是 `yundun-cert` 三条动作、不是 `oss:PutCname`，因此**不能**拿它充当 #20。
